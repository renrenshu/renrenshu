...
public partial class page_lxy_personal_IM_C_right : System.Web.UI.Page
{
    int id = 6;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            mandatory = Request.QueryString["mandatory"].ToString();
            lno = Request.QueryString["lno"].ToString();
        }
        getdata(pagenum);
        Page.DataBind();
    }
    ...
    public void getdata(int pagenum)
    {
        ...
        DataSet dsr, dsu;
        dsr = data.query("select client from RewardRelationship WHERE mandatory = '" + mandatory + "' AND lno = '" + lno + "'");

        // 取得总页数
        int a = dsr.Tables["base"].Rows.Count % 6;
        if (a > 0)
        {
            pagecount = dsr.Tables["base"].Rows.Count / 6 + 1;
        }
        else
        {
            pagecount = dsr.Tables["base"].Rows.Count / 6;
        }
        ...
        int num = 0;
        for (int i = ((pagecurrent - 1) * 6); i < dsr.Tables["base"].Rows.Count; i++)
        {
            dsu = data.query("select uname, uphone from UserInformation WHERE uid = '" + Convert.ToInt32(dsr.Tables["base"].Rows[i]["client"].ToString()) + "'");
            if (num >= 6)
            {
                break;
            }
    ...
    {
        Response.Redirect("personal_IM_C_right.aspx?pagenum=1");
    }
    ...
    protected void LastPage_Click(object sender, EventArgs e)
    {
        ...
        {
            pagecurrent--;
            string url_right = "personal_IM_C_right.aspx?pagenum=" + pagecurrent.ToString();
